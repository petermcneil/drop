...
import mcneil.peter.drop.listener.VerifiedEmailListener
...
class CreateAccountActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        auth.addAuthStateListener(VerifiedEmailListener(this))
    ...
    private fun sendEmailVerification() {
        ...
        user?.sendEmailVerification()?.addOnCompleteListener(this) { task ->
            progress.dismiss()
            if (task.isSuccessful) {
                Toast.makeText(this, getString(R.string.toast_email_sent, auth.currentUser!!.email), Toast.LENGTH_SHORT).show()
